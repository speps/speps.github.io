...
}`;class U{constructor(t,i){this.map={},this.id=t,this.spec=i;let s=Math.ceil(Math.sqrt(i.glyphs.length));this.width=s*i.width,this.height=s*i.height;for(let e=0;e<i.glyphs.length;e++){let r=e%s,a=Math.floor(e/s);this.map[i.glyphs[e]]=new n(r*i.width,a*i.height)}}}class E{constructor(t=0){this.index=t}}class O{constructor(t,i){this.location=t,this.value=i}}class Y{constructor(t){this.uniforms={},this.id=t}}class H{constructor(t,i,s,e){this.index=t,this.size=i,this.stride=s,this.offset=e}}class X{constructor(t){this.attribs=[],this.num=0,this.id=t}}class Z{constructor(t){this.fonts={},this.programs={},this.buffers={},this.gl=t,this.createProgram("background",N,F,{u_camera_position:new n(0,0),u_camera_scale:new n(1,1),u_time:0}),this.createProgram("cell",x,L,{u_resolution:new n(0,0),u_camera_position:new n(0,0),u_camera_scale:new n(1,1),u_position:new n(0,0),u_size:new n(1,1),u_time:0,u_direction:new n(0,-1),u_split:0,u_radius:0}),this.createProgram("particle",x,M,{u_resolution:new n(0,0),u_camera_position:new n(0,0),u_camera_scale:new n(1,1),u_position:new n(0,0),u_size:new n(1,1),u_time:0,u_direction:new n(0,-1),u_radius:0}),this.createProgram("debug_circle",x,q,{u_camera_position:new n(0,0),u_camera_scale:new n(1,1),u_position:new n(0,0),u_size:new n(1,1),u_thickness:0,u_color:new D(0,0,0,0)}),this.createProgram("font",x,V,{u_camera_position:new n(0,0),u_camera_scale:new n(1,1),u_position:new n(0,0),u_size:new n(1,1),u_font:new E(0),u_char_pos:new n(0,0),u_char_size:new n(0,0)}),this.createBuffer("screen",[[2,16,0],[2,16,8]],[-1,-1,0,0,3,-1,2,0,-1,3,0,2]),this.createBuffer("quad",[[2,16,0],[2,16,8]],[-.5,-.5,0,0,.5,-.5,1,0,.5,.5,1,1,-.5,-.5,0,0,.5,.5,1,1,-.5,.5,0,1]),this.loadFont("debug",I)}createProgram(t,i,s,e){const r=new Y(this.gl.createProgram());if(r.id===null)throw"createProgram";let a=(g,u)=>{const w=this.gl.createShader(g);if(w===null)throw"createShader";if(this.gl.shaderSource(w,u),this.gl.compileShader(w),!this.gl.getShaderParameter(w,this.gl.COMPILE_STATUS))throw`compileShader:${this.gl.getShaderInfoLog(w)}:${u}`;return w};const h=G+i.replace(`#version 300 es
precision highp float;
`,""),f=a(this.gl.VERTEX_SHADER,h),v=j+s.replace(`#version 300 es
precision highp float;
`,""),y=a(this.gl.FRAGMENT_SHADER,v);if(this.gl.attachShader(r.id,f),this.gl.attachShader(r.id,y),this.gl.linkProgram(r.id),!this.gl.getProgramParameter(r.id,this.gl.LINK_STATUS))throw`linkProgram:${this.gl.getProgramInfoLog(r.id)}`;for(let g in e){const u=this.gl.getUniformLocation(r.id,g);u===null&&console.warn(`getUniformLocation:${t}:${g}`),r.uniforms[g]=new O(u,e[g])}this.programs[t]=r}createBuffer(t,i,s){const e=new X(this.gl.createBuffer());if(e.id===null)throw"createBuffer";this.gl.bindBuffer(this.gl.ARRAY_BUFFER,e.id),this.gl.bufferData(this.gl.ARRAY_BUFFER,new Float32Array(s),this.gl.STATIC_DRAW);let r=0;for(let a=0;a<i.length;a++){const h=i[a];r+=h[0],e.attribs.push(new H(a,h[0],h[1],h[2]))}e.num=s.length/r,this.buffers[t]=e}loadFont(t,i){const s=new U(this.gl.createTexture(),i);if(s.id===null)throw"createTexture";fetch(s.spec.data).then(e=>{e.blob().then(r=>{createImageBitmap(r).then(a=>{this.gl.activeTexture(this.gl.TEXTURE0),this.gl.bindTexture(this.gl.TEXTURE_2D,s.id),this.gl.texParameteri(this.gl.TEXTURE_2D,this.gl.TEXTURE_MIN_FILTER,this.gl.LINEAR),this.gl.texParameteri(this.gl.TEXTURE_2D,this.gl.TEXTURE_MAG_FILTER,this.gl.LINEAR),this.gl.texParameteri(this.gl.TEXTURE_2D,this.gl.TEXTURE_WRAP_S,this.gl.CLAMP_TO_EDGE),this.gl.texParameteri(this.gl.TEXTURE_2D,this.gl.TEXTURE_WRAP_T,this.gl.CLAMP_TO_EDGE),this.gl.texImage2D(this.gl.TEXTURE_2D,0,this.gl.RGBA8,this.gl.RGBA,this.gl.UNSIGNED_BYTE,a),this.fonts[t]=s})})})}bindProgram(t){const i=this.programs[t];this.gl.useProgram(i.id);for(let s in i.uniforms){const e=i.uniforms[s];e.location!==null&&(typeof e.value=="number"&&this.gl.uniform1f(e.location,e.value),e.value instanceof n&&this.gl.uniform2f(e.location,e.value.x,e.value.y),e.value instanceof T&&this.gl.uniform3f(e.location,e.value.x,e.value.y,e.value.z),e.value instanceof D&&this.gl.uniform4f(e.location,e.value.x,e.value.y,e.value.z,e.value.w),e.value instanceof E&&this.gl.uniform1i(e.location,e.value.index))}}bindBuffer(t){const i=this.buffers[t];this.gl.bindBuffer(this.gl.ARRAY_BUFFER,i.id);for(let s=0;s<i.attribs.length;s++){const e=i.attribs[s];this.gl.enableVertexAttribArray(s),this.gl.vertexAttribPointer(s,e.size,this.gl.FLOAT,!1,e.stride,e.offset)}}drawBuffer(t,i){this.bindProgram(i),this.bindBuffer(t),this.gl.drawArrays(this.gl.TRIANGLES,0,this.buffers[t].num)}blendOff(){this.gl.disable(this.gl.BLEND)}blendPremul(){this.gl.enable(this.gl.BLEND),this.gl.blendFunc(this.gl.ONE,this.gl.ONE_MINUS_SRC_ALPHA)}drawScreenText(t,i,s,e){const r=this.fonts[t];r!==void 0&&(this.programs.font.uniforms.u_camera_position.value.set(o.currentViewWidth*.5,o.currentViewHeight*.5),this.programs.font.uniforms.u_camera_scale.value.set(o.currentViewWidth*.5,o.currentViewHeight*.5),this.programs.font.uniforms.u_size.value.set(r.spec.width,r.spec.height),this.programs.font.uniforms.u_char_size.value.set(r.spec.width,r.spec.height),this.drawText(r,i,s,r.spec.width,r.spec.height,e))}drawWorldText(t,i,s,e,r){const a=this.fonts[t];if(a===void 0)return;this.programs.font.uniforms.u_camera_position.value.setFrom(i),this.programs.font.uniforms.u_camera_scale.value.set(o.currentWorldWidth*.5,o.currentWorldHeight*.5);const h=a.spec.width/o.currentScale,f=a.spec.height/o.currentScale;this.programs.font.uniforms.u_size.value.set(h,f),this.programs.font.uniforms.u_char_size.value.set(a.spec.width,a.spec.height),this.drawText(a,s,e,h,f,r)}drawText(t,i,s,e,r,a){this.gl.activeTexture(this.gl.TEXTURE0),this.gl.bindTexture(this.gl.TEXTURE_2D,t.id);const h=i;i+=e*.5,s+=r*.5;for(let f of a){if(f==`
`){i=h+e*.5,s+=r;continue}if(f=="\0"){i+=e;continue}const v=f.codePointAt(0);if(v!==void 0){const y=t.map[v];y!==void 0&&(this.programs.font.uniforms.u_position.value.set(i,s),this.programs.font.uniforms.u_char_pos.value.setFrom(y),this.drawBuffer("quad","font"))}i+=e}}}class p{static halton(t,i){let s=0,e=1/i;for(;t>0;){let r=t%i;s+=r*e,t=(t-r)/i,e/=i}return s}static lerp(t,i,s){return t+(i-t)*s}static clamp(t,i,s){return t<i?i:t>s?s:t}static ease(t){return t=t-1,t*t*t*t}static rad(t){return t*Math.PI/180}}class A{constructor(){this.position=new n(0,0),this.direction=new n(0,-1),this.size=0,this.timerDivide=0,this.timerChain=0,this.timerDead=0,this.dead=!1,this.rotation=0,this.blinking=!1,this.blinkShow=!0,this.blinkTimer=0,this.radius=0,this.influence=0,this.chainInfluence=0,this.deadInfluence=0,this.updateValues()}updateValues(){this.radius=p.lerp(l.minRadius,l.maxRadius,this.size),this.influence=this.radius+p.lerp(l.minInfluence,l.maxInfluence,this.size),this.chainInfluence=p.lerp(0,this.influence,p.clamp(this.timerChain/l.chainDelay,0,1)),this.deadInfluence=p.lerp(0,this.influence,p.clamp(this.timerDead/l.deadDelay,0,1)),!this.dead&&this.size>=1&&(this.dead=!0)}setInitialValues(){this.timerDivide=l.divideDelay,this.timerChain=l.chainDelay,this.size=.5}canChain(){return this.timerChain<l.chainDelay}dying(){return this.dead&&this.timerDead<l.deadDelay}die(){this.dead=!0,this.timerChain=l.chainDelay}canDivide(){return!this.isDead()&&!this.dying()&&this.timerDivide>=l.divideDelay}divide(){this.timerDivide=0,this.timerChain=0}isDividing(){return this.timerDivide<l.divideDelay}isDead(){return this.dead&&this.timerDead>=l.deadDelay}update(t,i){i||(this.dead?this.timerDead+=t:(this.size=p.clamp(this.size+l.sizeGrowth*t,0,1),this.position.add(this.direction.copy().scale(p.lerp(l.minVelocity,l.maxVelocity,p.ease(this.size))*t)),this.timerDivide+=t,this.timerChain+=t)),(1-this.size)/l.sizeGrowth<l.feedbackDeathDelay?(this.blinking||(this.blinking=!0),this.blinkTimer-=t,this.blinkTimer<0&&(this.blinkShow=!this.blinkShow,this.blinkTimer=l.feedbackDeathBlinkDelay)):this.blinkShow=!0,this.rotation+=l.rotationRate,this.updateValues()}}class C{constructor(){this.position=new n}reset(){this.position.set(0,0)}cameraCoords(t){return new n((t.x-this.position.x)*o.currentScale+o.currentViewWidth*.5,(t.y-this.position.y)*o.currentScale+o.currentViewHeight*.5)}worldCoords(t){return new n((t.x-o.currentViewWidth*.5)/o.currentScale+this.position.x,(t.y-o.currentViewHeight*.5)/o.currentScale+this.position.y)}}class K{constructor(){this.points=new Array,this.camera=new C,this.lookAt=new n,this.cells=new Array,this.cellsToDivide=new Array,this.particles=new Array;for(let t=0;t<o.numBackgroundPoints;t++){const i={pos:new n(p.halton(t,2)-.5,p.halton(t,3)-.5),alpha:p.halton(t,5)};this.points.push(i)}}clear(){this.camera.reset(),this.lookAt.set(0,0),this.cells=[],this.cellsToDivide=[],this.particles=[]}generate(){this.clear();let t=new A;t.setInitialValues(),t.updateValues(),this.cells.push(t)}click(t,i){let s=this.camera.worldCoords(new n(t,i)),e=this.getClosestAt(s);return!!(e!==null&&n.distance(s,e.position)<=e.radius&&this.divideCell(e))}getClosestAt(t){let i=Number.MAX_VALUE,s=null;for(let e of this.cells)if(!e.dead){let r=n.squaredDistance(e.position,t);(!i||r<i)&&(i=r,s=e)}return s}divideCell(t){if(!t.canDivide())return!1;for(let h=0;h<this.cells.length;h++)if(this.cells[h]===t){this.cells.splice(h,1);break}let i=p.lerp(l.minAngle,l.maxAngle,t.size),s=p.lerp(l.minRadius,l.maxRadius,t.size/2)+1,e=new n(-t.direction.y*s,t.direction.x*s),r=new A;r.size=t.size/2,r.position=t.position.copy().sub(e),r.direction=t.direction.rotated(p.rad(-i)),this.cells.push(r);let a=new A;return a.size=t.size/2,a.position=t.position.copy().add(e),a.direction=t.direction.rotated(p.rad(i)),this.cells.push(a),!0}processCellsToDivide(){for(;this.cellsToDivide.length>0;)this.divideCell(this.cellsToDivide[0]),this.cellsToDivide.splice(0,1)}updateCellCollisions(){for(let s=0;s<this.cells.length;s++)for(let e=s+1;e<this.cells.length;e++){let r=this.cells[s],a=this.cells[e],h=n.squaredDistance(r.position,a.position),f=r.radius+a.radius;if(h<f*f){let v=n.sub(a.position,r.position);v.normalize();let y=f-Math.sqrt(h),g=1/r.radius,u=1/a.radius,w=g/(g+u),b=u/(g+u);r.position.sub(n.scale(v,y*w)),a.position.add(n.scale(v,y*b));let R=n.sub(a.direction,r.direction);n.dot(R,v)<0&&(r.direction.sub(n.scale(v,w)),r.direction.normalize(),a.direction.add(n.scale(v,b)),a.direction.normalize())}}let t=-o.refWidth/2,i=o.refWidth/2;for(let s=0;s<this.cells.length;s++){let e=this.cells[s],r=!1;e.position.x-e.radius<t&&(e.position.x=t+e.radius,e.direction.x<0&&(r=!0)),e.position.x+e.radius>i&&(e.position.x=i-e.radius,e.direction.x>0&&(r=!0)),r&&(e.direction.x=-e.direction.x)}}updateCellInteractions(){for(let t=0;t<this.cells.length;t++)for(let i=0;i<this.cells.length;i++){if(t==i)continue;let s=this.cells[t],e=this.cells[i];if(s.dying()){let r=n.distance(s.position,e.position);s.deadInfluence+e.radius>r&&e.die()}else if(s.canChain()&&e.canDivide()){let r=n.distance(s.position,e.position);if(s.chainInfluence+e.radius>r){let a=!1;for(let h=0;h<this.cellsToDivide.length;h++){let f=this.cellsToDivide[h];if(e===f){a=!0;break}}a||this.cellsToDivide.push(e)}}}}updateCamera(){let t=0;for(let i=0;i<this.cells.length;i++){let s=this.cells[i];t=Math.min(t,s.position.y)}this.lookAt.y+=(t-this.lookAt.y)*o.cameraStiffness,this.camera.position.setFrom(this.lookAt)}update(t){let i=[];for(let s=0;s<this.cells.length;s++){let e=this.cells[s];e.update(t,!1),e.isDead()&&i.push(s)}for(let s=i.length-1;s>=0;s--)this.cells.splice(i[s],1);this.updateCellCollisions(),this.updateCellInteractions(),this.processCellsToDivide(),this.updateCamera()}draw(t){t.programs.background.uniforms.u_camera_position.value.setFrom(this.camera.position),t.programs.background.uniforms.u_camera_scale.value.set(o.currentWorldWidth*.5,o.currentWorldHeight*.5),t.blendOff(),t.drawBuffer("screen","background"),t.blendPremul(),t.programs.cell.uniforms.u_camera_position.value.setFrom(this.camera.position),t.programs.cell.uniforms.u_camera_scale.value.set(o.currentWorldWidth*.5,o.currentWorldHeight*.5);for(let i=0;i<this.cells.length;i++){let s=this.cells[i];t.programs.cell.uniforms.u_position.value.setFrom(s.position),t.programs.cell.uniforms.u_size.value.set(s.radius*2,s.radius*2),t.programs.cell.uniforms.u_direction.value.setFrom(s.direction),t.programs.cell.uniforms.u_split.value=p.clamp((s.timerDivide-l.divideDelay)/l.divideAnimDuration,0,1),t.programs.cell.uniforms.u_radius.value=l.maxRadius,s.blinkShow&&!s.dying()&&t.drawBuffer("quad","cell")}for(let i=0;i<this.cells.length;i++){let s=this.cells[i];t.programs.debug_circle.uniforms.u_camera_position.value.setFrom(this.camera.position),t.programs.debug_circle.uniforms.u_camera_scale.value.set(o.currentWorldWidth*.5,o.currentWorldHeight*.5),t.programs.debug_circle.uniforms.u_position.value.setFrom(s.position),s.chainInfluence<s.influence&&(t.programs.debug_circle.uniforms.u_size.value.set(s.chainInfluence*2,s.chainInfluence*2),t.programs.debug_circle.uniforms.u_thickness.value=1,t.programs.debug_circle.uniforms.u_color.value.set(0,.5,0,1),t.drawBuffer("quad","debug_circle")),s.dying()&&(t.programs.debug_circle.uniforms.u_size.value.set(s.deadInfluence*2,s.deadInfluence*2),t.programs.debug_circle.uniforms.u_thickness.value=1,t.programs.debug_circle.uniforms.u_color.value.set(1,0,0,1),t.drawBuffer("quad","debug_circle"))}t.blendPremul(),t.programs.particle.uniforms.u_camera_position.value.setFrom(this.camera.position),t.programs.particle.uniforms.u_camera_scale.value.set(o.currentWorldWidth*.5,o.currentWorldHeight*.5);for(let i=0;i<this.particles.length;i++){let s=this.particles[i];t.programs.particle.uniforms.u_position.value.setFrom(s.position),t.programs.particle.uniforms.u_size.value.set(5,5),t.programs.particle.uniforms.u_direction.value.setFrom(n.normalize(s.velocity)),t.programs.particle.uniforms.u_radius.value=10,t.drawBuffer("quad","particle")}t.drawScreenText("debug",0,0,"microbobs")}}class Q{}class z extends Q{constructor(t,i){super(),this.x=t,this.y=i}}class _{constructor(t){this.state=t}enter(){this.state.manager.switchTo(this.state)}}class J{constructor(){this.events=[],this.world=new K,this.menu=new _(new $(this)),this.game=new _(new tt(this))}switchTo(t){this.current&&this.current!==t&&this.current.onExit(),this.current=t,t.onEnter()}click(t,i){this.events.push(new z(t,i))}isRunning(t){return this.current===t}update(t){if(this.current){for(;this.events.length>0;){const i=this.events[0];this.current.handle(i),this.events.splice(0,1)}this.current.update(t)}}draw(t){this.current&&this.current.draw(t)}}class ${constructor(t){this.manager=t}onEnter(){this.manager.world.generate()}onExit(){}handle(t){t instanceof z&&this.manager.world.click(t.x,t.y)&&this.manager.game.enter()}update(t){}draw(t){this.manager.world.draw(t),t.drawWorldText("debug",this.manager.world.camera.position,-48/o.currentScale,-4.5*16/o.currentScale,`tap to start
\0experiment
\0\0#7872304

\0\0>\0\0\0\0\0\0<`)}}class tt{constructor(t){this.manager=t}onEnter(){}onExit(){}handle(t){t instanceof z&&this.manager.world.click(t.x,t.y)}update(t){this.manager.world.update(t)}draw(t){this.manager.world.draw(t)}}{let d=function(){f=[window.innerWidth,window.innerHeight],o.resize(f[0],f[1]);const u=o.currentViewWidth,w=o.currentViewHeight;e.style.left=f[0]/2-u/2+"px",e.style.top=f[1]/2-w/2+"px",e.style.width=u+"px",e.style.height=w+"px",e.width=u,e.height=w},t=function(u){h.click(u.offsetX,u.offsetY),u.preventDefault()},i=function(u){u.preventDefault()},s=function(u){for(requestAnimationFrame(s),g+=(u-v)/1e3,v=u;g>=y;)h.update(y),g-=y;a.programs.background.uniforms.u_time.value=u,a.programs.cell.uniforms.u_resolution.value.set(e.width,e.height),a.programs.cell.uniforms.u_time.value=u,r.viewport(0,0,e.width,e.height),r.clear(r.COLOR_BUFFER_BIT),h.draw(a)};const e=document.createElement("canvas");document.body.appendChild(e);const r=e.getContext("webgl2");r.clearColor(1,1,0,1);const a=new Z(r),h=new J;h.menu.enter();let f=[1,1];window.addEventListener("resize",d,!1),window.addEventListener("pointerdown",t),window.addEventListener("touchstart",i,{passive:!1}),d();let v=performance.now();const y=1/30;let g=0;requestAnimationFrame(s)}
